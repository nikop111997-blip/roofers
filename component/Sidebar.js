...
const NavItem = ({ href, icon: Icon, label, collapsed, active = false }) => (
  ...
  >
    <div className="shrink-0 flex items-center justify-center">
      <Icon 
        size={20} 
        className={active ? "text-gray-600" : "text-gray-300 group-hover:text-gray-600"} 
      />
    </div>
    <span
...
export default function Sidebar({ session, collapsed, setCollapsed }) {
  ...
  return (
    <div
      className={`h-screen bg-[#005b38] border-r border-gray-200 transition-all duration-300 flex flex-col shadow-[4px_0_24px_rgba(0,0,0,0.02)]
      ${collapsed ? "w-20" : "w-64"}`}
    >
      {/* Header & Toggle */}
      ...
      <nav className="flex-1 py-6 space-y-1.5 overflow-y-auto overflow-x-hidden scrollbar-hide">
        ...
        {session?.user?.role === 'admin' && (
         ...
         {session?.user?.role === 'admin' && (
          <NavItem 
            ...
            label="Testimonials" 
            collapsed={collapsed} 
            active={pathname.includes("/dashboard/testimonials")}
          />
        )}
        <NavItem 
        ...
        <NavItem 
          href="/dashboard/categories" // Update these paths to match your actual routing
          icon={FileText} 
